...
library(OptM)
library(plyr)
source("C:/TEXAS A &M/aandmtexas/academics/analysis/gross/Supplemental_Code/VCF_files_filters/neutral snp_treemix/18 pops redo/redo/final_runs_m5/TreeMix_functions.R") #path to required functions for this analysis

########################################################
############# (A) Test migration events ################
########################################################

folder <- file.path(path="C:/TEXAS A &M/aandmtexas/academics/analysis/gross/Supplemental_Code/VCF_files_filters/neutral snp_treemix/18 pops redo/redo/test_migrations")                     #path to files of TreeMix replicates with different migration edges (m) to test
...
setwd("C:/TEXAS A &M/aandmtexas/academics/analysis/gross/Supplemental_Code/VCF_files_filters/neutral snp_treemix/18 pops redo/redo/final_runs_m5")                                             #folder with all TreeMix outputs from the final runs
maxLL("5neutralscriptpop18_5m_finalrun_", nt=30)                                         #first argument is stem of TreeMix output files, nt = number of runs
...
cfTrees("5neutralscriptpop18_5m_finalrun_", nt=30, p=1, m='PH85')                        #m is the method to calculate pairwise distances between unique trees (default is Robinson-Foulds distance)
...
pdf("TreeMix_output2.pdf")                                          
treemix.bootstrap("5neutralscriptpop18_5m_finalrun_2", out.file = "tmp",                 #stem of TreeMix files (as above) + number of run with highest likelihood and unique topology
                  phylip.file = "5neutralscriptpop18_finalconstree.newick",           #consensus tree in newick format (from the bootstrap procedure generated with PHYLIP)    
...
treemix.drift(in.file = "5neutralscriptpop18_5m_finalrun_2",                             #pairwise matrix for drift estimates with specified order of pops 
              pop.order.color.file = "poporder.txt") + 
              title("Drift")     

plot_resid("5neutralscriptpop18_5m_finalrun_2",                                          #pairwise matrix for residuals
