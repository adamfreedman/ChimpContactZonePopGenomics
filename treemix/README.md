...
* For generating TreeMix input file, Thom Nelson's adaptation (2019) of Tomaz Berisa's script to convert plink clustered allele frequencies to treemix format
* for TreeMix pipeline: Dahms, C. (2021) (All scripts and codes)
...
# calculate allele frequency based on poporder.list which contains all the population names as rows
plink --tfile plinkfileoutput --freq --within poporder.list 
# compress output file
gzip plink.frq.strat
module purge
module load GCC/8.3.0  CUDA/10.1.243  OpenMPI/3.1.4
module load  Biopython/1.75-Python-3.7.4
#use the python script to generate TreeMix input file
python2 plink2treemix.py plink.frq.strat.gz treemix.gz 
```
### 2. Build consensus tree with multiple migration events
Run `Step1_TreeMix.sh` in the command line, providing an input file, maximum number of cores, block size, outgroup (or 'noRoot' for unrooted trees), number of bootstrap replicates, path to PHYLIP consense program, output file name, range of migration events (m) and their number of replicates, for example:
...
In our case the input file is treemix_neutral.gz and we had 10 cores, 100 block size, noRoot as unrooted tree, the specified path for PHYLIP consense program, neutral_18pop as output file, 10 migration events and 10 replicates. This builds a consensus tree from bootstraps and adds a specified range of m. Tree replicates arestored in the *test_migrations* folder.

### 3. Test migration edges with OptM in R

Set working directory to the *test_migrations* folder and run the R package OptM (step A) from the R script `Step2&4_TreeMix.R`. This helps identify the optimum number of m.
